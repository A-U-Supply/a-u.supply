...
**Branch:** `latent-slideshow` (plan) → implementation PRs below
**Status:** shipped — PRs #577, #578, #579, #580 (all merged 2026-07-26)

Implementation PRs, in the order they landed:

1. **[#577]** the viewer: auto-hiding chrome, and Latents finally opens it
2. **[#578]** backend: slot slideshow order, named latent slideshows
3. **[#579]** slot card UI: `[▦ Slideshow (n)]` accordion, View all
4. **[#580]** Latent-level Slideshows section: several named slideshows

Landed alongside: **[#581]** — `src/lib/anchoredPanel.ts` plus a lint rule
that fails the build when a component hand-rolls `position: fixed` without
`portal.ts` or `anchoredPanel.ts`. Not part of this plan, but it is the guard
that stops the overlay bugs this feature kept tripping over.

**Deferred, still owed:** PDF export of a slot, with a picker preview before
export — the user's idea, explicitly held back until this landed.

**Known and not fixed:** on a *faced* slot card the head action buttons render
dark-on-dark (`.slot--face-*` sets `.slot__head`'s colour but `.action-btn`
carries its own). Pre-existing since faces shipped; noted in #579.
