...
def _ensure_mp3_preview(source_path: Path) -> Path | None:
    """Return a cached MP3 preview of `source_path`, rendering it if absent.

    Cache lives next to the source file as `<stem>_og.mp3` (same sibling
    convention as `<stem>_thumb.webp` image previews). Returns None when
    ffmpeg is missing or the transcode fails — callers should fall back
    to the raw file in that case so the route never 500s.
    """
    cache_path = source_path.with_name(source_path.stem + "_og.mp3")
...
@router.get(
    ...
    tags=["Media Items"],
    summary="Public audio preview for link unfurling",
    include_in_schema=False,
)
def get_media_og_audio(media_id: str, db: Session = Depends(get_db)):
    """Serve a public MP3 preview of an audio item for link-unfurling bots.

    Slack classic unfurling only media-unfurls **direct** media URLs, and
    only when the content is reasonable in size + content-type. The raw
    audio files in this catalog are large WAVs (often hundreds of MB); a
    192 kbps MP3 sibling (cached next to the source as `<stem>_og.mp3`)
    is small enough for Slack to fetch and standard enough for it to
    recognize as media and render its built-in audio player.

    Sibling to ``/media/{media_id}/og-thumb``. Only audio items are
    served; image/video items 404, keeping the public surface limited
    to audio shares. Falls back to the raw file (inline, with the
    original content-type) if ffmpeg is unavailable or transcode fails
    — the route never 500s purely because the preview couldn't be made.
    """
    item = db.query(MediaItem).filter(MediaItem.id == media_id).first()
    if not item:
        raise HTTPException(status_code=404, detail="Not found")
    ...
    preview_path = _ensure_mp3_preview(file_path)
    if preview_path is not None:
        return FileResponse(
            preview_path,
            media_type="audio/mpeg",
            headers={
                "Content-Disposition": f'inline; filename="{stem}.mp3"',
                "Cache-Control": "public, max-age=86400, immutable",
            },
        )

    # Transcode failed or ffmpeg missing — serve the raw file as a fallback
    # so callers still get audio bytes (may be too large for Slack, but
    # this keeps the endpoint honest for other clients).
    return FileResponse(
        file_path,
