...
class TestExtractUrls:
    ...
    def test_finds_youtube_url(self):
        from server.slack_scraper import extract_urls
...
class TestIsDownloadableUrl:
    ...
    def test_youtube_is_downloadable(self):
        from server.slack_scraper import is_downloadable_url
...
class TestScrapeChannel:
    ...
    def test_scrape_channel_processes_files(self, db_session):
        ...
        from server.slack_scraper import scrape_channel
        ...
        with patch("server.slack_scraper.get_channel_history", return_value=mock_history), \
             patch("server.slack_scraper.download_slack_file", return_value=True), \
             patch("server.slack_scraper._ingest_file", return_value={"status": "new", "media_item_id": "test-id"}), \
             patch("server.slack_scraper.SessionLocal", return_value=db_session), \
    ...
    def test_dedup_by_slack_file_id(self, db_session):
        ...
        with patch("server.slack_scraper.get_channel_history", return_value=mock_history), \
             patch("server.slack_scraper.download_slack_file") as mock_dl, \
             patch("server.slack_scraper.SessionLocal", return_value=db_session), \
             patch.object(db_session, "close"):
            result = scrape_channel("image-gen", "C_TEST")
...
class TestReactionRefresh:
    ...
    def test_refresh_reactions_updates_counts(self, db_session):
        from server.slack_scraper import refresh_reactions
        ...
        with patch("server.slack_scraper.get_reactions", return_value=mock_reactions_resp), \
             patch("server.slack_scraper.SessionLocal", return_value=db_session), \
             patch.object(db_session, "close"):
...
class TestMockYtDlp:
    ...
    def test_download_url_returns_none_on_failure(self):
        from server.slack_scraper import download_url
        from pathlib import Path
        import tempfile

        with tempfile.TemporaryDirectory() as tmpdir:
            with patch("server.slack_scraper.subprocess.run") as mock_run:
