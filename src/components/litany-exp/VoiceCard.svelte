<script lang="ts">
  ...
  interface Props {
    ...
    poolEntries: { name: string; source: string; pinned: boolean }[];
...
>
  <div class="left-col">
    ...
    <div class="sample-name">
      ...
      {:else}
        <span class="current-name" title={currentSampleName}
          >{currentSampleName || '(no sample)'}</span
        >
      {/if}
    </div>
  ...
  <div class="right-col">
      ...
      >
        ...
        {#if selOpen && selectedStep != null && getStepOverride('pitch') != null}
          <button
            ...
            onclick={clearStepOverrides}>&times;</button
          >
        {/if}
      </label>
      <button
    ...
    {#if selOpen && selectedStep != null}
      ...
      <div class="override-bar">
        <button
          ...
          onclick={clearStepOverrides}>&times; CLR</button
        >
    ...
    <div class="meta-row">
      ...
      <button
        ...
        title={poolOpen ? 'Close pool drawer' : 'Open pool drawer'}
        onclick={() => (poolOpen = !poolOpen)}
      >
        POOL {poolOpen ? '\u25B4' : '\u25BE'}
        <span class="pool-count">{poolEntryCount}</span>
      </button>
      <button
        class="brutalist-control meta-btn"
